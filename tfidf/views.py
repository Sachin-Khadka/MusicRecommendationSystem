...
from client.models import Music, Review
from . import contentbased as cb


def export_csv(request):
  # Create the HttpResponse object with the appropriate CSV header.
  data = download_csv(request, Music.objects.all())
  response = HttpResponse(data, content_type='text/csv')
  return response


def recommendation(request):
    music_data = download_csv(request, Music.objects.all())
    review_data = download_csv(request, Music.objects.all())
    music_response = HttpResponse(music_data, content_type='text/csv')
    review_response = HttpResponse(review_data, content_type='text/csv')
    return music_response
    return review_response
    musics = pd.read_csv(music_response)
    usics = pd.read_sql_
    ratings = pd.read_csv("SELECT * from shop_review", connection)
